...
for i in range(len(x_step)):
    ...
    y2.append(voigt(x_step[i], [k, 1000, 3.5, 0.6, 0, 350]))

y_e = np.sqrt(y2)
p1 = [np.min(y2),  np.max(y2)-np.min(y2), 3.5, 0.9, 1, 50]
#p1 = [10, 40000, 3.5, 0.6, 0, 350]

m = Minuit.from_array_func(chi2, p1, error=(10, 10, 0.01, 0.001, .1, 1),
                                   limit=(None, None, None, (0.01, 5), None, None),
...
ax1.errorbar(x_step, y2, yerr=y_e , fmt='ro', ecolor='black', capsize=5, zorder=1)
ax1.plot(x_fit, y_fit,  color='#3392FF', zorder=2)
