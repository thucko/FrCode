'''
Version 4 of data analysis program Inculding Voigt profile
Add the ability to choose between forward and backward scans
Author: Tim Hucko
...
if prompt1 is False:
    ...
    for index, row in list_file.iterrows():
        ...
        # Saving figures
        if fig_save is True:
            ggplot.save(g1, filename='%s_scan_%d_%s_fit_%s.pdf' % (sel2, q, sel, volt),
                    path=fig_pwd)
            ggplot.save(g2, filename='%s_scan_residuals_%d_%s_fit_%s.pdf' % (sel2, q, sel, volt),
                    path=fig_pwd)
        q = q + 1
